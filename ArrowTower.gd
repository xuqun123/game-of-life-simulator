...
export var arrow_rotate_x = -2
export var arrow_velocities_x = [2, 4]
...
func _ready():	
	...
	animation_player = get_node("WeaponBallista/AnimationPlayer")
...
func arrow_detect_collision(collision):
	if collision:
		...
		if collider.name == "Robot":
			...
			if grid.started:
				robot.kill_robot()
		elif collider.name && collider.name != "ArrowTower":
			reset_bow()
